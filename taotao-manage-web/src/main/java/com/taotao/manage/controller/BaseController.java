 ...
 * 只是一个父类，写了通用的异常处理方法。
 * 
 * @ExceptionHandler(Exception.class)的作用： 执行子类的方法时出了异常，springmvc会先执行子类自己的异常处理机制，
 * 	如果执行的方法既没有try-catch，子类中又没有对应的@ExceptionHandler注解的异常处理方法，
 * 	那么springmvc就会到父类中查找是否有@ExceptionHandler注解的异常处理方法，
 * 	如果父类中也没有，那么springmvc就会找是否有被@ControllerAdvice注解的类（这种类是用于全局异常处理），再看这个类中是否有对应的异常处理方法。
 */
